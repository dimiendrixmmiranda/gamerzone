export const listaDeTimes = [
    {
        id: 'semclube',
        time: 'Sem Clube',
        abreviacao: 'SAI',
        regiao: '',
        logo: '/default/logo-default.png',
        cor: '#FFFFFF',
        colocacao: '999',
        elenco: []
    },
    {
        id: 'imperial',
        time: 'Imperial',
        abreviacao: 'IMP',
        regiao: 'am',
        logo: 'https://i.imgur.com/3HXz0Ql.png',
        cor: '#088408',
        colocacao: '34',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: 'Rifler',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                },
                configs: {
                    mouse: {
                        ['Nome']: 'Pulsar Xlite V3 Es Black',
                        ['DPI']: '400',
                        ['Sensitivity']: '4',
                        ['eDPI']: '1600.00',
                        ['Zoom Sensitivity']: '1.00',
                        ['HZ']: '1000',
                        ['Windows Sensitivity']: '6'
                    },
                    crosshair: {
                        ['Style']: 'Classic Static',
                        ['Follow Recoil']: 'No',
                        ['Dot']: 'No',
                        ['Length']: '3',
                        ['Thickness']: '0.1',
                        ['Gap']: '-3',
                        ['Outline']: 'No',
                        ['Color']: 'Custom',
                        ['Red']: '0',
                        ['Green']: '255',
                        ['Blue']: '0',
                        ['Alpha']: 'Yes',
                        ['Alpha Value']: '255',
                        ['T Style']: 'No',
                        ['Deployed Weapon Gap']: 'No',
                        ['Sniper Width']: '0',
                        ['Código da Mira']: 'CSGO-qbtPc-D6heq-R7z58-CuJAf-LwAkE',
                    },
                    viewmodel: {
                        ['FOV']: '60',
                        ['Offset X']: '2.5',
                        ['Offset Y']: '0',
                        ['Offset Z']: '1.5',
                        ['Presetpos']: '0',
                        ['Código do Viewmodel']: 'viewmodel_fov 60; viewmodel_offset_x 2.5; viewmodel_offset_y 0; viewmodel_offset_z 1.5; viewmodel_presetpos 0;',
                    },
                    lauchOptions: {
                        ['Código Launch Options']: '-tickrate 128 -freq 240 -novid -high -console -nojoy'
                    },
                    video: {
                        ['Resolution']: '1280x960',
                        ['Aspect Radio']: '5:4',
                        ['Scaling Mode']: 'Black Bars',
                        ['Brightness']: '93%',
                        ['Display Mode']: 'Fullscreen',
                    },
                    advancedVideo: {
                        ['Boost Player Contrast']: 'Enabled',
                        ['V-Sync']: 'Disabled',
                        ['NVIDIA Reflex Low Latency']: '',
                        ['NVIDIA G-Sync']: '',
                        ['Maximum FPS In Game']: '',
                        ['Multisampling Anti-Aliasing Mode']: 'None',
                        ['Global Shadow Quality']: '',
                        ['Dynamic Shadows']: '',
                        ['Model / Texture Detail']: 'Low',
                        ['Texture Filtering Mode']: 'Bilinear',
                        ['Shader Detail']: '',
                        ['Particle Detail']: '',
                        ['Ambient Occlusion']: '',
                        ['High Dynamic Range']: '',
                        ['FidelityFX Super Resolution']: '',
                    },
                    hud: {
                        ['HUD Scale']: '0.85',
                        ['HUD Color']: 'Team Color'
                    },
                    radar: {
                        ['Radar Centers The Player']: 'No',
                        ['Radar is Rotating']: 'Yes',
                        ['Toggle Shape With Scoreboard']: 'Yes',
                        ['Radar Hud Size']: '1.00',
                        ['Radar Map Zoom']: '0.4',
                    }
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Vinicius "VINI" Figueiredo tem 26 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: 'IGL',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                },
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: 'Rifler',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como Awper na Imperial',
                linkHLTV: '',
                posicao: 'Awper',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'Rifler',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Rafael "zakk" Fernandes tem 34 anos e atua como Coach na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'furia',
        time: 'Fúria',
        abreviacao: 'FUR',
        regiao: 'am',
        logo: 'https://i.imgur.com/Uw0sWaU.png',
        cor: '#36383D',
        colocacao: '1',
        elenco: [
            {
                nome: 'Gabriel Toledo',
                nick: 'FalleN',
                linkHLTV: '',
                posicao: '',
                idade: '34',
                imagem: 'https://i.imgur.com/zBNUQni.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Yuri Santos',
                nick: 'yuurih',
                linkHLTV: '',
                posicao: '',
                idade: '25',
                imagem: 'https://i.imgur.com/HiAYbww.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Mareks Gaļinskis',
                nick: 'YEKINDAR',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/ibGjNZs.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Kaike Cerato',
                nick: 'KSCERATO',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/oi2xaRw.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Danil Golubenko',
                nick: 'molodoy',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/318ZhPP.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Sid Macedo',
                nick: 'sidde',
                linkHLTV: '',
                posicao: 'coach',
                idade: '28',
                imagem: 'https://i.imgur.com/XQtF7NE.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: '9z',
        time: '9z',
        abreviacao: '9z',
        regiao: 'am',
        logo: 'https://i.imgur.com/3yPceZd.png',
        cor: '#083E7F',
        colocacao: '44',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: 'Rifler',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'Bestia',
        time: 'bestia',
        abreviacao: 'BES',
        regiao: 'am',
        logo: 'https://i.imgur.com/Dd2kd3s.png',
        cor: '#0879BD',
        colocacao: '88',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'bountyhunters',
        time: 'Bounty Hunters',
        abreviacao: 'BHT',
        regiao: 'am',
        logo: 'https://i.imgur.com/uSBYXoz.png',
        cor: '#FDC202',
        colocacao: '70',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'crashers',
        time: 'Crashers',
        abreviacao: 'CRA',
        regiao: 'am',
        logo: 'https://i.imgur.com/iagMEHY.png',
        cor: '#FF0918',
        colocacao: '186',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'fluxo',
        time: 'Fluxo',
        abreviacao: 'FLX',
        regiao: 'am',
        logo: 'https://i.imgur.com/HAcV5WT.png',
        cor: '#652CE2',
        colocacao: '37',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'gaimingladiotors',
        time: 'Gaimin Gladiotors',
        abreviacao: 'GLT',
        regiao: 'am',
        logo: 'https://i.imgur.com/veUfNUI.png',
        cor: '#2645CC',
        colocacao: '75',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'galorys',
        time: 'Galorys',
        abreviacao: 'GAL',
        regiao: 'am',
        logo: 'https://i.imgur.com/mNwNouq.png',
        cor: '#590F7F',
        colocacao: '89',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'keydstars',
        time: 'Keyd Stars',
        abreviacao: 'KYS',
        regiao: 'am',
        logo: 'https://i.imgur.com/dEPAU6W.png',
        cor: '#A4478D',
        colocacao: '90',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'legacy',
        time: 'Legacy',
        abreviacao: 'LEG',
        regiao: 'am',
        logo: 'https://i.imgur.com/LVajJ1G.png',
        cor: '#FEC433',
        colocacao: '14',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'mibr',
        time: 'MIBR',
        abreviacao: 'MIBR',
        regiao: 'am',
        logo: 'https://i.imgur.com/wu1U5hl.png',
        cor: '#324C70',
        colocacao: '45',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'oddik',
        time: 'ODDIK',
        abreviacao: 'ODK',
        regiao: 'am',
        logo: 'https://i.imgur.com/Lx9Z0aY.png',
        cor: '#13161A',
        colocacao: '132',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'pain',
        time: 'Pain',
        abreviacao: 'PAIN',
        regiao: 'am',
        logo: 'https://i.imgur.com/GAsNsTd.png',
        cor: '#FF3333',
        colocacao: '10',
        elenco: [
            {
                nome: 'Rodrigo Bittencourt',
                nick: 'biguzera',
                linkHLTV: 'http://hltv.org/player/18141/biguzera',
                posicao: 'IGL',
                idade: '28',
                imagem: 'https://i.imgur.com/bioMbQZ.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Lucas Soares',
                nick: 'nqz',
                linkHLTV: 'https://www.hltv.org/player/19694/nqz',
                posicao: 'AWP',
                idade: '20',
                imagem: 'https://i.imgur.com/QHCLHq2.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'João Vinicius',
                nick: 'snow',
                linkHLTV: 'https://www.hltv.org/player/20171/snow',
                posicao: 'Entry',
                idade: '18',
                imagem: 'https://i.imgur.com/bDR4JRa.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Franco Garcia',
                nick: 'dgt',
                linkHLTV: 'https://www.hltv.org/player/14736/dgt',
                posicao: 'Lurker',
                idade: '24',
                imagem: 'https://i.imgur.com/PxrDIeq.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/zybUXki.png'
                }
            },
            {
                nome: 'David Tapia Maldonado',
                nick: 'dav1deuS',
                linkHLTV: 'https://www.hltv.org/player/18865/dav1deus',
                posicao: 'Entry',
                idade: '25',
                imagem: 'https://i.imgur.com/WtF1Zlw.png',
                nacionalidade: {
                    pais: 'Chile',
                    imagem: 'https://i.imgur.com/UzSqDRF.png'
                }
            },
            {
                nome: 'Iuri Gomes',
                nick: 'RaaiiN',
                linkHLTV: 'https://www.hltv.org/coach/24429/raaiin',
                posicao: 'coach',
                idade: '28',
                imagem: 'https://i.imgur.com/it4pAeJ.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'redcanids',
        time: 'Red Canids',
        abreviacao: 'RED',
        regiao: 'am',
        logo: 'https://i.imgur.com/vu0Uolr.png',
        cor: '#DE0A08',
        colocacao: '62',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'yawara',
        time: 'Yawara',
        abreviacao: 'YAW',
        regiao: 'am',
        logo: 'https://i.imgur.com/j9puOQq.png',
        cor: '#a59d04',
        colocacao: '118',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'fnatic',
        time: 'Fnatic',
        abreviacao: 'FNT',
        regiao: 'eu',
        logo: 'https://i.imgur.com/F09plYH.png',
        cor: '#FFA808',
        colocacao: '27',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'themongols',
        time: 'The Mongols',
        abreviacao: 'TMG',
        regiao: 'as',
        logo: 'https://i.imgur.com/zy87RXw.png',
        cor: '#EEBD5D',
        colocacao: '5',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'vitality',
        time: 'Vitality',
        abreviacao: 'VIT',
        regiao: 'eu',
        logo: 'https://i.imgur.com/xSXYwyH.png',
        cor: '#F1CF16',
        colocacao: '2',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'falcons',
        time: 'Falcons',
        abreviacao: 'FAL',
        regiao: 'eu',
        logo: 'https://i.imgur.com/L6slGyS.png',
        cor: '#088408',
        colocacao: '3',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'mouz',
        time: 'Mouz',
        abreviacao: 'MOUZ',
        regiao: 'eu',
        logo: 'https://i.imgur.com/E8novi3.png',
        cor: '#DF0A22',
        colocacao: '4',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'spirit',
        time: 'Spirit',
        abreviacao: 'SPI',
        regiao: 'eu',
        logo: 'https://i.imgur.com/vvgjgi3.png',
        cor: '#415E69',
        colocacao: '6',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'natusvincere',
        time: 'Natus Vincere',
        abreviacao: 'NAVI',
        regiao: 'eu',
        logo: 'https://i.imgur.com/VTDwUQi.png',
        cor: '#FFE008',
        colocacao: '7',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'g2',
        time: 'G2',
        abreviacao: 'G2',
        regiao: 'eu',
        logo: 'https://i.imgur.com/bnas4zJ.png',
        cor: '#161D7D',
        colocacao: '8',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'aurora',
        time: 'Aurora',
        abreviacao: 'AUR',
        regiao: 'eu',
        logo: 'https://i.imgur.com/9GpKiHd.png',
        cor: '#278DA5',
        colocacao: '9',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'astralis',
        time: 'Astralis',
        abreviacao: 'AST',
        regiao: 'eu',
        logo: 'https://i.imgur.com/2r0kooI.png',
        cor: '#BF080C',
        colocacao: '11',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'fazeclan',
        time: 'Faze Clan',
        abreviacao: 'FAZE',
        regiao: 'eu',
        logo: 'https://i.imgur.com/prEm4AW.png',
        cor: '#BF080C',
        colocacao: '12',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: '3dmax',
        time: '3dMax',
        abreviacao: '3DM',
        regiao: 'eu',
        logo: 'https://i.imgur.com/YrKf0Uz.png',
        cor: '#24262B',
        colocacao: '13',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'b8',
        time: 'B8',
        abreviacao: 'B8',
        regiao: 'eu',
        logo: 'https://i.imgur.com/hwOxlly.png',
        cor: '#24262B',
        colocacao: '16',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'gamerlegion',
        time: 'Gamerlegion',
        abreviacao: 'GL',
        regiao: 'eu',
        logo: 'https://i.imgur.com/gTWxynA.png',
        cor: '#181F86',
        colocacao: '17',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'heroic',
        time: 'Heroic',
        abreviacao: 'HER',
        regiao: 'eu',
        logo: 'https://i.imgur.com/kpsDA1g.png',
        cor: '#B20C15',
        colocacao: '18',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'virtuspro',
        time: 'Virtus Pro',
        abreviacao: 'VP',
        regiao: 'eu',
        logo: 'https://i.imgur.com/nj18FsE.png',
        cor: '#FF680A',
        colocacao: '19',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'parivision',
        time: 'Parivision',
        abreviacao: 'PAR',
        regiao: 'eu',
        logo: 'https://i.imgur.com/MphI9C5.png',
        cor: '#12A9FB',
        colocacao: '25',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'lynnvision',
        time: 'Lynn Vision',
        abreviacao: 'LYV',
        regiao: 'as',
        logo: 'https://i.imgur.com/bR1rEXn.png',
        cor: '#EEBD5D',
        colocacao: '23',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'tyloo',
        time: 'Tyloo',
        abreviacao: 'TYL',
        regiao: 'as',
        logo: 'https://i.imgur.com/FayVi3l.png',
        cor: '#FF090B',
        colocacao: '28',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
    {
        id: 'rareatom',
        time: 'Rare Atom',
        abreviacao: 'RAR',
        regiao: 'as',
        logo: 'https://i.imgur.com/4SEgToO.png',
        cor: '#7C18A8',
        colocacao: '47',
        elenco: [
            {
                nome: 'Marcelo Cespedes',
                time: 'imperial',
                nick: 'chelo',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '27',
                imagem: 'https://i.imgur.com/oWtAaTx.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Vinicius Figueiredo',
                time: 'imperial',
                nick: 'VINI',
                descricao: 'Marcelo "chelo" Cespedes tem 27 anos e atua como rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '26',
                imagem: 'https://i.imgur.com/edQvFhY.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Felipe Medeiros',
                time: 'imperial',
                nick: 'skullz',
                descricao: 'Felipe "skullz" Medeiros tem 23 anos e atua como IGL na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '23',
                imagem: 'https://i.imgur.com/qE3jr6d.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Santino Rigal',
                time: 'imperial',
                nick: 'try',
                descricao: 'Santino "try" Rigal tem 21 anos e atua como AWP na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '21',
                imagem: 'https://i.imgur.com/xeg94qQ.png',
                nacionalidade: {
                    pais: 'Argentina',
                    imagem: 'https://i.imgur.com/XEZCrfG.png'
                }
            },
            {
                nome: 'Kaiky Santos',
                time: 'imperial',
                nick: 'noway',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: '',
                idade: '20',
                imagem: 'https://i.imgur.com/P6zBWyz.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
            {
                nome: 'Rafael Fernandes',
                nick: 'zakk',
                descricao: 'Kaiky "noway" Santos tem 20 anos e atua como Rifler na Imperial',
                linkHLTV: '',
                posicao: 'coach',
                idade: '34',
                imagem: 'https://i.imgur.com/XyE2XBh.png',
                nacionalidade: {
                    pais: 'Brasil',
                    imagem: 'https://i.imgur.com/dGRGEY7.png'
                }
            },
        ]
    },
]