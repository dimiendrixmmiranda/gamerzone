...
export default function Page() {
    ...
    const [blogAtual, setBlogAtual] = useState<Blog | null>(null)
    ...
    async function enviarComentario() {
        if (!comentario.trim() || !usuario || !blogAtual) return;
        ...
        try {
            const ref = doc(db, "listaDeColunas", blogAtual.id);
            await updateDoc(ref, { comentarios: arrayUnion(comentarioObj) });

            setBlogAtual({
                ...blogAtual,
                comentarios: [...(blogAtual.comentarios ?? []), comentarioObj]
            });
            setComentario('');
        } catch (error) {
    ...
    async function toggleLike(index: number) {
        if (!usuario || !blogAtual) return;
        const uid = usuario.uid;
        const comentarios = [...(blogAtual.comentarios ?? [])];
        ...
        try {
            const ref = doc(db, "listaDeNoticias", blogAtual.id);
            await updateDoc(ref, { comentarios });
            setBlogAtual({ ...blogAtual, comentarios });
        } catch (error) {
    ...
    async function responder(e: React.FormEvent) {
        e.preventDefault();
        if (!usuario || !blogAtual || !responderComentario.trim() || comentarioIndexRespondido === null) return;
        ...
        <Template paginaClube={false}>
            <div className="p-4 md:p-8 flex flex-col">
                <div className="w-full border-b-2 border-black overflow-hidden">
                    <div className="relative w-[180px] h-[180px] rounded-full overflow-hidden -mb-6 mx-auto md:w-[240px] md:h-[240px]">
                        <Image alt="Imagem do blog" src={`/blogs/${blogAtual?.nomeDaColuna}.png`} fill className="object-cover" />
                    </div>
                </div>
                <div className="flex flex-col gap-4">
                    <div className="text-center">
                        <h1 className="capitalize font-bold text-3xl md:text-5xl">{blogAtual?.nomeDaColuna}</h1>
                        <span>Por {blogAtual?.autor}</span>
                    </div>
                    <div className="flex flex-col gap-2 md:gap-4">
                        <h3 className="text-2xl font-bold text-center md:text-4xl">{blogAtual?.titulo}</h3>
                        {
                            blogAtual && (
                                <div className="relative w-full h-[300px] mx-auto bg-zinc-800 md:h-[450px] md:max-w-[800px] lg:h-[550px] lg:max-w-[1000px]">
                                    <Image alt="Imagem" src={blogAtual?.imagem} fill className="object-cover"/>
                                </div>
                            )
                        }
                        <ul className="flex flex-col gap-2 max-w-[1000px] mx-auto">
                            {
                                blogAtual?.conteudo.map((p, i) => {
                                    return (
                                        <li key={i}>
                                            <p className="indent-6 md:indent-12 lg:text-lg">{p}</p>
                                        </li>
                                    )
                                })
                            }
                ...
                <div className="flex flex-col gap-2">
                    <h3 className="text-xl font-bold">
                        Comentários ({blogAtual?.comentarios?.length ?? 0})
                    </h3>
                    <p className="text-xs">
                    ...
                    <ul className="flex flex-col gap-4 mt-4">
                        {blogAtual != undefined && blogAtual?.comentarios?.length > 0 &&
                            blogAtual?.comentarios
